...
| `Custom Event Properties` | Map your custom event properties by setting their field names in **Event properties name** and adding the field name holding the **Value** in the related field. E.g. if you input`size`in the **Event properties name** and`properties.items.0.product.size` **\[1]** in **Value**, you will have a custom event property called`size`with a value based on the content of the field`properties.items.0.product.size`. You also have the option to set a static string/numeric value in **Value**. |
